...
// remove similar edges in a Graph
// Ex:
// Edge 1-0 is the same as 0-1, so this function will remove either of those
std::vector<std::vector<ushort>> remove_duplicate_edges(Graph& g);

// Write out .dot files for graphviz to draw 
void write_graphs_to_file(std::string file, std::vector<std::vector<ushort>> adjList);
